...
class PanopticDeepLabSemSegHead(DeepLabV3PlusHead):
    ...
    def forward(self, features, targets=None, weights=None, ood_mask=None):
        ...
        if self.training:
            return None, self.losses(y, targets, weights, y_ood, ood_mask)
        else:
            y = F.interpolate(
    ...
    def losses(self, predictions, targets, weights=None, predictions_ood=None, ood_mask=None):
...
@SEM_SEG_HEADS_REGISTRY.register()
class PanopticDeepLabSigmoidSemSegHead(DeepLabV3PlusHead):
    """
    A semantic segmentation head described in :paper:`Panoptic-DeepLab`.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_depthwise_separable_conv:
            ...
            weight_init.c2_xavier_fill(self.head[0])
            weight_init.c2_xavier_fill(self.head[1])
        self.predictor = Conv2d(head_channels, num_classes, kernel_size=1, activation=torch.sigmoid)
        nn.init.normal_(self.predictor.weight, 0, 0.001)
        nn.init.constant_(self.predictor.bias, 0)
