...
def training_routine(args, network, dataset_cfg):
    ...
    dataset_train = CityscapesOOD(root=config.cityscapes_ood_path, split=config.split, cfg=dataset_cfg,
                                  transform=build_sem_seg_train_aug(dataset_cfg), random_img_path=args.random_img_path)
    ...
    if comm.is_main_process():
        ...
        print("Removing dir: ", args.random_img_path)
        os.rmdir(args.random_img_path)
...
def generateRandomImages(args):
    random_img_path = args.random_img_path
    while(True):
        no_of_images = len(os.listdir(random_img_path))
        if no_of_images < 200:
            height = 1024
            width = 2048
            random_instances = random.randint(2, 6)
            ood_mask = np.zeros((3, height, width), dtype="uint8")
            for j in range(random_instances):
                ...
                im_out[im_out == 255] = j + 1
                ood_mask[:, start_height:start_height + h, start_width:start_width + w] = im_out

            image_name = str(uuid.uuid4()) + ".png"
            #print(os.getpid(), "creating image: ", image_name, no_of_images)
            Image.fromarray(ood_mask.astype(np.uint8).transpose((1, 2, 0))).save(
                os.path.join(random_img_path, image_name))

        else:
            print("Random_length : ", no_of_images)
            time.sleep(1)
...
if __name__ == '__main__':
    ...
    args.random_img_path = config.random_image_dir
    args.dist_url = 'tcp://127.0.0.1:64486'
    print("Command Line Args:", args)
    process = multiprocessing.Process(target=generateRandomImages, args=(args,))
    process.start()
