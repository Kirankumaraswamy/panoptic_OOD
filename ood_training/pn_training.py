...
batch_center_loss = 0
batch_offset_loss = 0
batch_pos_semantic_loss = 0
batch_neg_semantic_loss = 0
iteration = 1
...
def train_model(network, dataloader_train, optimizer, scheduler, epoch=None):
    ...
    total_loss = 0
    total_loss_center = 0
    total_loss_pos_seg = 0
    total_loss_offset = 0
    total_loss_neg_seg = 0
    network = network.train()

    global batch_loss
    global batch_center_loss
    global batch_offset_loss
    global batch_pos_semantic_loss
    global batch_neg_semantic_loss
    global iteration
    for i, (x, target) in enumerate(dataloader_train):
        ...
        total_loss_pos_seg += loss_dict_reduced['loss_pos_seg']
        total_loss_offset += loss_dict_reduced['loss_offset']
        total_loss_neg_seg += loss_dict_reduced['loss_neg_seg']
        batch_loss += losses_reduced
        batch_center_loss += loss_dict_reduced['loss_center']
        batch_offset_loss += loss_dict_reduced['loss_offset']
        batch_pos_semantic_loss += loss_dict_reduced['loss_pos_seg']
        batch_neg_semantic_loss += loss_dict_reduced['loss_neg_seg']
        ...
        if comm.is_main_process():
            ...
            if (iteration + 1) % 20 == 0:
                with open("./status_" + config.suffix + ".txt", "a") as f:
                    f.write(
                        "\nEpoch {}, iteration: {}, total_loss: {}, loss_pos_sem_seg: {}, loss_center: {}, loss_offset: {}, loss_neg_sem_seg: {}, lr: {}".format(
                            epoch, iteration, batch_loss / 20, batch_pos_semantic_loss / 20, batch_center_loss / 20,
                            batch_offset_loss / 20, batch_neg_semantic_loss/20, scheduler.get_last_lr()))
                batch_loss = 0
                batch_center_loss = 0
                batch_offset_loss = 0
                batch_pos_semantic_loss = 0
    ...
    loss = {
        ...
        "loss_pos_sem_seg": total_loss_pos_seg / (i + 1),
        "loss_center": total_loss_center / (i + 1),
        "loss_offset": total_loss_offset / (i + 1),
        "loss_neg_sem_seg": total_loss_neg_seg / (i + 1)
    }
    return loss
...
def training_routine(args, network, dataset_cfg):
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        losses_pos_seg.append(train_loss["loss_pos_sem_seg"])
        losses_center.append(train_loss["loss_center"])
        losses_offset.append(train_loss["loss_offset"])
        losses_neg_seg.append(train_loss["loss_neg_sem_seg"])
        ...
        if comm.is_main_process():
            ...
            fig = plt.figure("Pos semantic loss " + str(epoch))
            plt.plot(x_values, losses_pos_seg)
            plt.title("Pos semantic loss")
            fig.savefig("./pos_semantic_loss_" + config.suffix + ".png")
            ...
            fig = plt.figure("Neg semantic loss " + str(epoch))
            plt.plot(x_values, losses_neg_seg)
            plt.title("Neg semantic loss")
            fig.savefig("./neg_semantic_loss_" + config.suffix + ".png")
    ...
    if comm.is_main_process():
        ...
        print(losses_pos_seg)
        print(losses_neg_seg)
        print(losses_center)
        print(losses_offset)
