...
demo_folder = os.path.join(root_folder, opts.demo_folder)
save_folder = os.path.join(root_folder, opts.save_folder)
...
for idx, image in enumerate(images):
    ...
    ood_data = np.array(anomaly_map)

    ood_data[np.where(ood_data > 127)] = 255
    ood_data[np.where(ood_data <= 127)] = 0
    ood_image = Image.fromarray(ood_data)
    ood_image.save(os.path.join(anomaly_path, "ood_"+ basename))
