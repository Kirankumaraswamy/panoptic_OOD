...
def apply_blending(mask, bdd_web_panoptic_values, bdd_depth_img, bdd_masked_image,
    ...
    city_height = bdd_original.shape[0]
    city_width = bdd_original.shape[1]
    ...
    for id in panoptic_ids:
        ...
        blurred_img = cv2.GaussianBlur(bdd_masked_image, (kernel_size, kernel_size), 0)
...
def create_bdd_web_panoptic(args):
    ...
    if not os.path.exists(os.path.join(outputFolder, parent_folder, status_file_name)):
    ...
    if os.path.exists(outFile):
        ...
        images = d["images"]
        for index, web_id in enumerate(web_img_Ids):
    ...
    for progress, f in enumerate(bdd_files):
        ...
        image_id = fileName.split("_gtFine_instanceIds.png")[0]
        if image_id in completed_ids:
            continue

        random.seed(instance_count)
        originalFormat = np.array(Image.open(f))
        city_width = originalFormat.shape[1]
        city_height =originalFormat.shape[0]
        ...
        for sample_instance_count in range(sample_instances):
            ...
            for i in range(random_instances):
                ...
                if segment_name  in web_categories:
                    location_type = web_categories[segment_name][0]
                else:
                    location_type = web_categories["unknown"][0]
                ...
                if segment_name == "unknown":
                    # we want to pick the scale value randomly
                    # since this code was added after creating validation set
                    # it will affect the random seed if we introduce one more random call
                    cityscape_relative_scale_factor = (road_random_pixel / len(road_pixels[0])) * 2
                    cityscape_relative_scale_factor = max(0.1, cityscape_relative_scale_factor)
                else:
...
def main():
    ...
    parser.add_argument("--bdd-folder",
                        ...
                        default="/home/kumarasw/OOD_dataset/bdd_new/bdd",
                        type=str)
