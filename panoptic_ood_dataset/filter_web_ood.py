...
def display_to_filter_images(args):
    ...
    ood_extract_rgb_files = glob.glob(os.path.join(ood_extract_rgb_dir, "*", "*"))
    ...
    while len(non_filtered_list) > 0:
        ...
        ood_image = cv2.imread(os.path.join(oodExtractFolder, category, image_id+".jpeg"))

        if len(ood_image.shape) <= 2:
            count += 1
            continue
        ...
        panoptic_image = np.ones((shape[0], shape[1]), dtype="uint16") * 5001

        panoptic_image = panoptic_image * foreground_mask
        semantic_image = semantic_image * foreground_mask

        horizontal = np.concatenate(( rgb_foreground, foreground_ood_image), axis=1)
        cv2.imshow(image_id, horizontal)
